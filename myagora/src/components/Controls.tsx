...
const MoreButton = (props: {fields: ToolbarMoreButtonDefaultFields}) => {
  ...
  // 14. AI Summarizer
  actionMenuitems.push({
    componentName: 'summarizer',
    order: 14,
    icon: 'chat',
    iconColor: isSummarizerOpen ? $config.PRIMARY_ACTION_BRAND_COLOR : $config.SECONDARY_ACTION_COLOR,
    textColor: $config.FONT_COLOR,
    title: isSummarizerOpen ? 'Hide Summarizer' : 'AI Summarizer',
    onPress: () => {
      setActionMenuVisible(false);
      setIsSummarizerOpen(!isSummarizerOpen);
    },
  });
